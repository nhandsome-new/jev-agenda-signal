...
speech-to-text and Jev calls as they happen.*
...
We also tried sending only the last 3 minutes plus the previous signals. It
scored 82% (vs. 98% with the whole transcript); most errors were 🟢 falling back
to 🔴 once the conclusion left the window.

- **Could help:** long meetings (about an hour or more), where the whole
  transcript strains the context limit, accuracy, or cost.
- **To make it work:** keep the signals in code and let Jev judge only what
  changed in the window. Not implemented.
