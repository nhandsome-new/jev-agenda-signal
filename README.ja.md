...
会議テキスト全体の代わりに、直近3分の会話と直前の信号だけを送る方式も試しました。韓国語の台本
5本で正解と一致した判定は 410 / 500（82%）で、全体を送る場合（98%）より低くなりました。最も多い
誤り（90件中44件）は、結論が3分の区間の外に出ると、直前の信号が 🟢 だったにもかかわらず Jev が
🔴 と答えたケースです。Jev は目の前の内容を判定するだけなので、前の状態を覚えて引き継がせる
方式はうまくいきませんでした。

それでも、会議テキスト全体を送ることが負担になる状況では、区間方式のほうが有利な可能性があります。

- 長い会議（おおよそ1時間以上）：全体のテキストが Jev の入力上限（3.2万トークン）に近づく場合や、
  関係のない内容が長く積み重なって精度が下がる場合。
- 長い会議で頻繁に判定する場合：毎回全体を送るとコストと遅延が判定回数分だけ膨らみますが、
  区間方式なら呼び出しごとの入力が小さく一定です。

うまく機能させるには、状態はコード側で持つ必要があります。Jev は直近の区間で起きたこと
（言及なし／議論した／結論が出た／結論を撤回）だけを判定し、コードが信号を更新します。
言及がなければ信号はそのままにします。この方式は実装・検証していません。
